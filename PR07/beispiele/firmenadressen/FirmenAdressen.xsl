...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="/">
		<html>
			...
			<body>
				<h1>Alle Firmenadressen (Anzahl: <xsl:value-of select="count(/dataroot/tcaq_Adressen)"/>)</h1>
				<!-- <h1>Firmenadressen in Chemnitz (Anzahl: <xsl:value-of select="count(/dataroot/tcaq_Adressen[ORT='Chemnitz'])"/>)</h1> -->
				<!-- <h1>Firmenadressen Firmename beginnt mit 'C' (Anzahl: <xsl:value-of select="count(/dataroot/tcaq_Adressen[starts-with(FIRMA, 'C')])"/>)</h1> -->
				<table border="1">
					<tr>
						<th>Position</th>
						<th>ID</th>
	...
	<xsl:template name="firmenliste">
		...
		<xsl:for-each select="/dataroot/tcaq_Adressen">
			<!-- <xsl:for-each select="/dataroot/tcaq_Adressen[ORT='Chemnitz']"> -->
			<!-- <xsl:for-each select="/dataroot/tcaq_Adressen[starts-with(FIRMA,'C')]"> -->
